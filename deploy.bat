@echo off
setlocal
echo [1/5] Generating folder SEO meta + sitemap (from live R2 listing)...
pushd webapp
...
echo [3/5] Repairing Material icons (Flutter tree-shaking drops 2 icons used in the app)...
python tools\fix_icons_font.py
if errorlevel 1 goto :error

echo [4/5] Switching engine to same-origin in generated bootstrap...
