...
then writes:
  - webapp/web/seo_meta.json         keywords + folder slugs -> {title, desc, path}
  - webapp/web/sitemap.xml           root + 47 keyword URLs + every folder URL

The keyword entries (tools/keyword_meta.json) are the same copy the old
functions/_middleware.js served: tools/prerender_seo.py bakes both into
static build/web/<slug>/index.html pages after `flutter build`.
...
WEB_DIR = WEBAPP / "web"
KEYWORD_META = TOOLS_DIR / "keyword_meta.json"
SEO_META_OUT = WEB_DIR / "seo_meta.json"
...
def keyword_slugs():
    ...
    if not KEYWORD_META.exists():
        print(f"WARN: {KEYWORD_META.name} missing - no keyword URLs.", file=sys.stderr)
        return []
    with open(KEYWORD_META, encoding="utf-8") as f:
...
def generate():
    ...
    payload = {
        ...
        "keywords": {
            slug: {"title": kw_meta[slug]["title"], "desc": kw_meta[slug]["desc"]}
            for slug in sorted(kw_meta)
        },
        "folders": dict(sorted(entries.items())),
    }
    SEO_META_OUT.write_text(
        json.dumps(payload, ensure_ascii=False, indent=1),
        encoding="utf-8",
    )
    print(f"OK: {SEO_META_OUT.relative_to(WEBAPP)} ({len(payload['keywords'])} keywords + {len(payload['folders'])} folders)")
